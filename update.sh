...
# finished. Written to a fixed path outside RADPANEL_MAIN_FOLDER so it survives
...
xui_update_status_file="${XUI_UPDATE_STATUS_FILE:-/etc/radpanel/update-status.json}"
...
setup_ssl_certificate() {
    ...
    if [ $? -ne 0 ]; then
        ...
        echo -e "${yellow}Please ensure port 80 is open and try again later with: radpanel${plain}"
    ...
    ~/.acme.sh/acme.sh --installcert --force -d ${domain} \
        ...
        --reloadcmd "systemctl restart radpanel" > /dev/null 2>&1
    ...
    if [[ -f "$webCertFile" && -f "$webKeyFile" ]]; then
        ${xui_folder}/radpanel cert -webCert "$webCertFile" -webCertKey "$webKeyFile" > /dev/null 2>&1
...
setup_ip_certificate() {
    ...
    local reloadCmd="systemctl restart radpanel 2>/dev/null || rc-service radpanel restart 2>/dev/null || true"
    ...
    ${xui_folder}/radpanel cert -webCert "${certDir}/fullchain.pem" -webCertKey "${certDir}/privkey.pem"
    if [ $? -ne 0 ]; then
...
ssl_cert_issue() {
    local existing_webBasePath=$(${xui_folder}/radpanel setting -show true | grep 'webBasePath:' | awk -F': ' '{print $2}' | tr -d '[:space:]' | sed 's#^/##')
    local existing_port=$(${xui_folder}/radpanel setting -show true | grep 'port:' | awk -F': ' '{print $2}' | tr -d '[:space:]')
    ...
    systemctl stop radpanel 2> /dev/null || rc-service radpanel stop 2> /dev/null
    ...
    if [[ ${cert_exists} -eq 0 ]]; then
        ...
        if [ $? -ne 0 ]; then
            ...
            systemctl start radpanel 2> /dev/null || rc-service radpanel start 2> /dev/null
            return 1
        else
    ...
    reloadCmd="systemctl restart radpanel || rc-service radpanel restart"
    echo -e "${green}Default --reloadcmd for ACME is: ${yellow}systemctl restart radpanel || rc-service radpanel restart${plain}"
    echo -e "${green}This command will run on every certificate issue and renew.${plain}"
    read -rp "Would you like to modify --reloadcmd for ACME? (y/n): " setReloadcmd
    if [[ "$setReloadcmd" == "y" || "$setReloadcmd" == "Y" ]]; then
        echo -e "\n${green}\t1.${plain} Preset: systemctl reload nginx ; systemctl restart radpanel"
        echo -e "${green}\t2.${plain} Input your own command"
        echo -e "${green}\t0.${plain} Keep default reloadcmd"
        read -rp "Choose an option: " choice
        case "$choice" in
            1)
                echo -e "${green}Reloadcmd is: systemctl reload nginx ; systemctl restart radpanel${plain}"
                reloadCmd="systemctl reload nginx ; systemctl restart radpanel"
                ;;
            2)
                echo -e "${yellow}It's recommended to put radpanel restart at the end${plain}"
    ...
    if [[ "$setPanel" == "y" || "$setPanel" == "Y" ]]; then
        ...
        if [[ -f "$webCertFile" && -f "$webKeyFile" ]]; then
            ...
            systemctl restart radpanel 2> /dev/null || rc-service radpanel restart 2> /dev/null
        else
    ...
    case "$ssl_choice" in
        ...
        2)
            ...
            if [[ $release == "alpine" ]]; then
                rc-service radpanel stop > /dev/null 2>&1
            else
                systemctl stop radpanel > /dev/null 2>&1
            fi
            ...
            if [[ $release == "alpine" ]]; then
                rc-service radpanel restart > /dev/null 2>&1
            else
                systemctl restart radpanel > /dev/null 2>&1
            fi
        ...
        3)
            ...
            # 3.4 Apply Settings via radpanel binary
            ${xui_folder}/radpanel cert -webCert "$custom_cert" -webCertKey "$custom_key" > /dev/null 2>&1
            ...
            systemctl restart radpanel > /dev/null 2>&1 || rc-service radpanel restart > /dev/null 2>&1
            ;;
        4)
            echo ""
            ...
            if [[ "$bind_local" == "y" || "$bind_local" == "Y" ]]; then
                ${xui_folder}/radpanel setting -listenIP "127.0.0.1" > /dev/null 2>&1
                SSL_HOST="127.0.0.1"
...
config_after_update() {
    ...
    echo -e "${yellow}radpanel settings:${plain}"
    ${xui_folder}/radpanel setting -show true
    ${xui_folder}/radpanel migrate

    # Properly detect empty cert by checking if cert: line exists and has content after it
    local existing_cert=$(${xui_folder}/radpanel setting -getCert true 2> /dev/null | grep 'cert:' | awk -F': ' '{print $2}' | tr -d '[:space:]')
    local existing_port=$(${xui_folder}/radpanel setting -show true | grep -Eo 'port: .+' | awk '{print $2}')
    local existing_webBasePath=$(${xui_folder}/radpanel setting -show true | grep -Eo 'webBasePath: .+' | awk '{print $2}' | sed 's#^/##')
    ...
    if [[ ${#existing_webBasePath} -lt 4 ]]; then
        ...
        ${xui_folder}/radpanel setting -webBasePath "${config_webBasePath}"
...
# by invoking the freshly downloaded radpanel CLI. IP Limit is load-bearing on
...
setup_fail2ban() {
    ...
    if [[ ! -x /usr/bin/radpanel ]]; then
        echo -e "${yellow}radpanel CLI not found; skipping Fail2ban auto-setup.${plain}"
        return 0
    fi

    # Scripts older than v3.4.0 have no setup-fail2ban and exit 0 from the
    # usage banner, which would read as success here.
    if ! grep -q '"setup-fail2ban")' /usr/bin/radpanel; then
        echo -e "${yellow}This radpanel.sh predates 'radpanel setup-fail2ban'; skipping Fail2ban auto-setup.${plain}"
        return 0
    fi

    echo -e "${green}Setting up Fail2ban for the IP Limit feature...${plain}"
    if /usr/bin/radpanel setup-fail2ban; then
        echo -e "${green}Fail2ban setup complete.${plain}"
    else
        echo -e "${yellow}Fail2ban setup did not finish; IP Limit stays disabled until you run 'radpanel' and open the IP Limit menu. Continuing.${plain}"
    fi
    return 0
}

# Lands a systemd unit file at ${xui_service}/radpanel.service via a temp file +
...
# Older tags predate some of these files (radpanel.rc arrived in v2.8.4). Serving
...
require_repo_files() {
    ...
    [[ "${ref}" == "main" ]] && return 0
    for name in "$@"; do
        status=$(${curl_bin} -sIL --retry 3 --connect-timeout 15 -o /dev/null -w '%{http_code}' "https://raw.githubusercontent.com/radinmovafaghh-coder/radpanel/${ref}/${name}")
        if [[ "${status}" != "200" ]]; then
...
update_radpanel() {
    cd ${xui_folder%/radpanel}/

    load_xui_env

    if [ -f "${xui_folder}/radpanel" ]; then
        current_xui_version=$(${xui_folder}/radpanel -v)
        echo -e "${green}Current radpanel version: ${current_xui_version}${plain}"
    else
        _fail "ERROR: Current radpanel version: unknown"
    fi

    echo -e "${green}Downloading new radpanel version...${plain}"
    ...
    if [[ -n "${XUI_UPDATE_TAG}" ]]; then
        ...
        tag_version=$(${curl_bin} -Ls "https://api.github.com/repos/radinmovafaghh-coder/radpanel/releases/latest" 2> /dev/null | grep '"tag_name":' | sed -E 's/.*"([^"]+)".*/\1/')
        if [[ ! -n "$tag_version" ]]; then
            _fail "ERROR: Failed to fetch radpanel version, it may be due to GitHub API restrictions, please try it later"
        fi
    fi
    echo -e "Got radpanel latest version: ${tag_version}, beginning the installation..."
    # radpanel.sh, radpanel.rc and the unit files must come from the same release as
    ...
    local required_files=("radpanel.sh")
    [[ $release == "alpine" ]] && required_files+=("radpanel.rc")
    require_repo_files "${script_ref}" "${required_files[@]}"
    ${curl_bin} -fLRo ${xui_folder}-linux-$(arch).tar.gz https://github.com/radinmovafaghh-coder/radpanel/releases/download/${tag_version}/radpanel-linux-$(arch).tar.gz 2> /dev/null
    if [[ $? -ne 0 ]]; then
        _fail "ERROR: Failed to download radpanel, please be sure that your server can access GitHub"
    fi
    if [[ ! -s ${xui_folder}-linux-$(arch).tar.gz ]]; then
        rm ${xui_folder}-linux-$(arch).tar.gz -f > /dev/null 2>&1
        _fail "ERROR: Downloaded radpanel release archive is empty, please be sure that your server can access GitHub"
    fi
    ...
    rm -f "${archive}.sha256"
    sidecar_code=$(${curl_bin} -sL --retry 3 --retry-delay 3 --connect-timeout 15 --max-time 60 -o "${archive}.sha256" -w '%{http_code}' "https://github.com/radinmovafaghh-coder/radpanel/releases/download/${tag_version}/radpanel-linux-$(arch).tar.gz.sha256" 2> /dev/null)
    if [[ "${sidecar_code}" == "200" ]]; then
        ...
        _fail "ERROR: Failed to download the checksum for radpanel-linux-$(arch).tar.gz (HTTP ${sidecar_code})"
    fi

    if [[ -e ${xui_folder}/ ]]; then
        echo -e "${green}Stopping radpanel...${plain}"
        if [[ $release == "alpine" ]]; then
            if [ -f "/etc/init.d/radpanel" ]; then
                rc-service radpanel stop > /dev/null 2>&1
                rc-update del radpanel > /dev/null 2>&1
                echo -e "${green}Removing old service unit version...${plain}"
                rm -f /etc/init.d/radpanel > /dev/null 2>&1
            else
                rm radpanel-linux-$(arch).tar.gz -f > /dev/null 2>&1
                _fail "ERROR: radpanel service unit not installed."
            fi
        else
            if [ -f "${xui_service}/radpanel.service" ]; then
                systemctl stop radpanel > /dev/null 2>&1
                systemctl disable radpanel > /dev/null 2>&1
                echo -e "${green}Removing old systemd unit version...${plain}"
                rm ${xui_service}/radpanel.service -f > /dev/null 2>&1
                systemctl daemon-reload > /dev/null 2>&1
            else
                rm radpanel-linux-$(arch).tar.gz -f > /dev/null 2>&1
                _fail "ERROR: radpanel systemd unit not installed."
            fi
        fi
        # Kill any leftover mtg (MTProto) sidecars. radpanel runs them outside its own
        ...
        echo -e "${green}Removing old radpanel version...${plain}"
        rm ${xui_folder} -f > /dev/null 2>&1
        rm ${xui_folder}/radpanel.service -f > /dev/null 2>&1
        rm ${xui_folder}/radpanel.service.debian -f > /dev/null 2>&1
        rm ${xui_folder}/radpanel.service.arch -f > /dev/null 2>&1
        rm ${xui_folder}/radpanel.service.rhel -f > /dev/null 2>&1
        rm ${xui_folder}/radpanel -f > /dev/null 2>&1
        rm ${xui_folder}/radpanel.sh -f > /dev/null 2>&1
        ...
        rm radpanel-linux-$(arch).tar.gz -f > /dev/null 2>&1
        _fail "ERROR: radpanel not installed."
    fi

    echo -e "${green}Installing new radpanel version...${plain}"
    tar zxvf radpanel-linux-$(arch).tar.gz > /dev/null 2>&1
    if [[ $? -ne 0 ]]; then
        rm radpanel-linux-$(arch).tar.gz -f > /dev/null 2>&1
        _fail "ERROR: Failed to extract the radpanel release archive -- the previous installation has already been removed, so the panel will not start until this is fixed; try running the update again"
    fi
    rm radpanel-linux-$(arch).tar.gz -f > /dev/null 2>&1
    cd radpanel > /dev/null 2>&1
    if [[ $? -ne 0 || ! -s radpanel ]]; then
        _fail "ERROR: Extracted radpanel archive is missing the radpanel binary -- the previous installation has already been removed, so the panel will not start until this is fixed; try running the update again"
    fi
    chmod +x radpanel > /dev/null 2>&1
    ...
    chmod +x radpanel bin/xray-linux-$(arch) > /dev/null 2>&1
    ...
    echo -e "${green}Downloading and installing radpanel.sh script...${plain}"
    local xui_script_temp="/usr/bin/radpanel-temp.$$"
    rm -f "${xui_script_temp}"
    ${curl_bin} -fLRo "${xui_script_temp}" "https://raw.githubusercontent.com/radinmovafaghh-coder/radpanel/${script_ref}/radpanel.sh" > /dev/null 2>&1
    if [[ $? -ne 0 ]]; then
        rm -f "${xui_script_temp}"
        _fail "ERROR: Failed to download radpanel.sh script, please be sure that your server can access GitHub"
    fi
    if [[ ! -s "${xui_script_temp}" ]]; then
        rm -f "${xui_script_temp}"
        _fail "ERROR: Downloaded radpanel.sh script is empty, please be sure that your server can access GitHub"
    fi
    mv -f "${xui_script_temp}" /usr/bin/radpanel
    if [[ $? -ne 0 ]]; then
        rm -f "${xui_script_temp}"
        _fail "ERROR: Failed to install radpanel.sh script"
    fi

    chmod +x ${xui_folder}/radpanel.sh > /dev/null 2>&1
    chmod +x /usr/bin/radpanel > /dev/null 2>&1
    mkdir -p /var/log/radpanel > /dev/null 2>&1
    ...
    if [[ $release == "alpine" ]]; then
        echo -e "${green}Downloading and installing startup unit radpanel.rc...${plain}"
        xui_rc_temp="/etc/init.d/radpanel.tmp.$$"
        rm -f "${xui_rc_temp}"
        ${curl_bin} -fLRo "${xui_rc_temp}" "https://raw.githubusercontent.com/radinmovafaghh-coder/radpanel/${script_ref}/radpanel.rc" > /dev/null 2>&1
        if [[ $? -ne 0 ]]; then
            rm -f "${xui_rc_temp}"
            _fail "ERROR: Failed to download startup unit radpanel.rc, please be sure that your server can access GitHub"
        fi
        if [[ ! -s "${xui_rc_temp}" ]]; then
            rm -f "${xui_rc_temp}"
            _fail "ERROR: Downloaded startup unit radpanel.rc is empty, please be sure that your server can access GitHub"
        fi
        mv -f "${xui_rc_temp}" /etc/init.d/radpanel
        if [[ $? -ne 0 ]]; then
            rm -f "${xui_rc_temp}"
            _fail "ERROR: Failed to install startup unit radpanel.rc"
        fi
        chmod +x /etc/init.d/radpanel > /dev/null 2>&1
        chown root:root /etc/init.d/radpanel > /dev/null 2>&1
        rc-update add radpanel > /dev/null 2>&1
        rc-service radpanel start > /dev/null 2>&1
    else
        if [ -f "radpanel.service" ]; then
            echo -e "${green}Installing systemd unit...${plain}"
            if ! _install_xui_service_unit "radpanel.service" "false"; then
                echo -e "${red}Failed to copy radpanel.service${plain}"
                exit 1
            fi
        else
            ...
            case "${release}" in
                ubuntu | debian | armbian)
                    if [ -f "radpanel.service.debian" ]; then
                        echo -e "${green}Installing debian-like systemd unit...${plain}"
                        if _install_xui_service_unit "radpanel.service.debian" "false"; then
                            service_installed=true
                        fi
                    fi
                    ;;
                arch | manjaro | parch)
                    if [ -f "radpanel.service.arch" ]; then
                        echo -e "${green}Installing arch-like systemd unit...${plain}"
                        if _install_xui_service_unit "radpanel.service.arch" "false"; then
                            service_installed=true
                        fi
                    fi
                    ;;
                *)
                    if [ -f "radpanel.service.rhel" ]; then
                        echo -e "${green}Installing rhel-like systemd unit...${plain}"
                        if _install_xui_service_unit "radpanel.service.rhel" "false"; then
                            service_installed=true
                        fi
                    fi
                    ;;
            ...
            if [ "$service_installed" = false ]; then
                ...
                case "${release}" in
                    ubuntu | debian | armbian)
                        service_unit_url="https://raw.githubusercontent.com/radinmovafaghh-coder/radpanel/${script_ref}/radpanel.service.debian"
                        ;;
                    arch | manjaro | parch)
                        service_unit_url="https://raw.githubusercontent.com/radinmovafaghh-coder/radpanel/${script_ref}/radpanel.service.arch"
                        ;;
                    *)
                        service_unit_url="https://raw.githubusercontent.com/radinmovafaghh-coder/radpanel/${script_ref}/radpanel.service.rhel"
                        ;;
                esac

                if ! _install_xui_service_unit "$service_unit_url" "true"; then
                    echo -e "${red}Failed to install radpanel.service from GitHub (${script_ref}) -- the release tarball did not ship one either${plain}"
                    exit 1
                fi
            fi
        fi
        chown root:root ${xui_service}/radpanel.service > /dev/null 2>&1
        chmod 644 ${xui_service}/radpanel.service > /dev/null 2>&1
        systemctl daemon-reload > /dev/null 2>&1
        systemctl enable radpanel > /dev/null 2>&1
        systemctl start radpanel > /dev/null 2>&1
    fi
    ...
    echo -e "${green}radpanel ${tag_version}${plain} updating finished, it is running now..."
    echo -e ""
    echo -e "┌───────────────────────────────────────────────────────┐
