# radpanel — Architecture & Code Map
...
> tree rather than a pinned release (Go module `github.com/radinmovafaghh-coder/radpanel/v3`).
>
...
radpanel is a **web control panel for [Xray-core](https://github.com/XTLS/Xray-core)**. The Go
...
2. **The Runtime abstraction (multi-node).** A panel can manage remote "nodes" (other radpanel
  ...
  → POST {basePath}/panel/api/...
    → Gin engine (internal/web/web.go: initRouter)
      → middleware chain: SecurityHeaders → MaxBodyBytes (10 MiB; importDB exempt)
                          → [DomainValidator, if webDomain set] → gzip → sessions("radpanel")
...
├── install.sh / update.sh / radpanel.sh                        # VPS install + management CLI
├── radpanel.service.*  / radpanel.rc                               # systemd units (debian/rhel/arch) + rc script
...
A "node" (`model.Node`) is another radpanel instance this panel controls. Every state-changing
...
- **Module path is `.../v3`.** Internal imports use `github.com/radinmovafaghh-coder/radpanel/v3/internal/...`.
- **SQLite vs Postgres.** Default is SQLite at `{RADPANEL_DB_FOLDER}/radpanel.db`. Postgres via
