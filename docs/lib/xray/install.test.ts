...
describe('buildScriptCommand', () => {
  ...
  it('pins a specific version by passing the tag to master install.sh', () => {
    ...
    expect(cmd).toBe(
      'bash <(curl -Ls https://raw.githubusercontent.com/radinmovafaghh-coder/radpanel/master/install.sh) v3.4.1',
    );
  });
...
describe('buildDockerRun', () => {
  it('reflects fail2ban, port, and base path options', () => {
    ...
    expect(cmd).toContain('ghcr.io/radinmovafaghh-coder/radpanel:latest');
    expect(cmd).toContain('-v $PWD/db/:/etc/radpanel/');
  });
...
describe('buildDockerCompose', () => {
  it('produces valid-looking compose with the image and volumes', () => {
    ...
    expect(yaml).toContain('image: ghcr.io/radinmovafaghh-coder/radpanel:latest');
    expect(yaml).toContain('network_mode: host');
    expect(yaml).toContain("XUI_PORT: '2096'");
    expect(yaml).toContain('- ./db/:/etc/radpanel/');
  });
});
