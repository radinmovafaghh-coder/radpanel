---
title: Discord 机器人
description: 将 Discord 机器人接入 radpanel，在指定频道接收实时的面板事件 Embed 告警、周期性健康报告（含数据库备份）以及执行交互式控制命令。
icon: Bot
---

radpanel 提供了完整的 Discord 集成支持：通过事件总线（`EventBus`）实时推送事件告警、通过定时任务发送包含数据库备份的服务器状态报告，以及通过 Discord Gateway 执行交互式管理命令。
...
2. 点击右上角的 **New Application**，输入名称（例如 `radpanel Notifier`）并确认创建。
3. 在左侧菜单中，进入 **Bot** 标签页。
...
1. 在 radpanel 面板中，打开**面板设置** → **Discord 机器人**（或直接访问 `/settings#discord`）。
2. 在**通用**区域：
   - 开启**启用 Discord 通知**。
...
3. 在**通知**区域：
   ...
   - 如需自动备份，可开启**数据库备份**，定时报告中将自动附带 `radpanel.db` 备份文件。
...
| `!backup` | 立即导出并发送当前数据库备份文件（`radpanel.db`）与 `config.json`。 |
...
| `discordBotBackup` | `false` | 是否在周期性报告中自动附带数据库备份文件（`radpanel.db`）。 |
