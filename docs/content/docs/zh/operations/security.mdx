---
title: 安全加固
description: 加固 radpanel——面板认证与 2FA、Fail2ban IP 限制、防火墙规则、BBR，以及保持更新。
icon: ShieldCheck
---
...
IP 数量。限制的执行由 **Fail2ban** 负责，radpanel 会为你安装并配置好它（脚本安装时默认启用，
Docker 安装时通过 `XUI_ENABLE_FAIL2BAN=true` 启用）。

可从 `radpanel` 菜单（**22 — IP Limit Management**）进行管理：安装/配置、修改封锁时长
（默认 **30 分钟**）、封锁/解封某个 IP、查看封锁日志，以及检查状态。其底层机制如下：

- 该 jail 名为 **`3x-ipl`**；封锁日志位于 `/var/log/radpanel/3xipl.log` 与
  `/var/log/radpanel/3xipl-banned.log`（也可通过 `radpanel banlog` 查看）。
...
只开放你实际使用的端口：SSH、面板端口、订阅端口以及你的入站端口。`radpanel` 菜单
...
`radpanel` 菜单（**26 — Enable BBR**）用于切换 Google 的 BBR 拥塞控制算法
（`net.ipv4.tcp_congestion_control = bbr`、`net.core.default_qdisc = fq`），它在拥塞链路
上通常能提升吞吐量。

## 保持更新

定期更新 radpanel 和 Xray-core——安全修复会随新版本发布。请关注
[发布页面](https://github.com/radinmovafaghh-coder/radpanel/releases)，并参见
