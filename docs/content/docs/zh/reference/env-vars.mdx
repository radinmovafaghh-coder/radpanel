---
title: 环境变量
description: radpanel 的 XUI_* 环境变量完整参考——涵盖数据库、面板、日志、内存、节点令牌加密以及隧道健康监测器。
icon: Variable
---

radpanel 从 `XUI_*` 环境变量中读取其运行时配置。使用脚本安装时，安装程序会将
这些变量写入服务环境文件（`/etc/default/radpanel`，或视发行版而定的
`/etc/conf.d/radpanel` / `/etc/sysconfig/radpanel`）；使用 Docker 时，则在
`docker-compose.yml` 或 `docker run -e` 中设置。默认值已足够合理——只需设置
你需要更改的项，然后重启即可：`systemctl restart radpanel`。
...
| `RADPANEL_DB_FOLDER`          | `/etc/radpanel` | SQLite 数据库文件（`radpanel.db`）所在的文件夹。                         |
...
默认的 SQLite 数据库路径为 `/etc/radpanel/radpanel.db`。有关 SQLite ↔ PostgreSQL 的
...
| `XUI_NODE_TOKEN_KEY_FILE` | `/etc/radpanel/node_token_key.json` | JSON 密钥环，权限须为 `0600` 或更严格。优先加载。                                                                                                                      |
...
`keys`，把 `active` 指向它，保留旧密钥用于解密，然后再次运行 `radpanel encrypt-tokens`。
...
| `RADPANEL_LOG_FOLDER` | `/var/log/radpanel`  | 日志输出目录。                                              |
| `RADPANEL_BIN_FOLDER` | `bin`            | 存放 Xray-core 二进制文件及 geosite/geoip 文件的文件夹。    |
...
| -------------------- | -------------------------------------------------------------------------------------------- |
| `XUI_NONINTERACTIVE` | 设为 `1`（或在无 TTY 的环境下运行）即可在零提示的情况下完成安装；生成的凭据会写入 `/etc/radpanel/install-result.env`。参见 [安装](/docs/guide/installation#unattended--cloud-init)。 |
