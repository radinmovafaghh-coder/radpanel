---
title: 安装
description: 通过官方脚本（稳定版、指定版本或开发最新版）、无人值守 / cloud-init 或 Docker 安装 radpanel，并选择 SQLite 或 PostgreSQL。
icon: Download
---

radpanel 可运行在多种 Linux 发行版上——Ubuntu、Debian、Armbian、
...
安装完成后，记下打印出的登录信息并运行 `radpanel` 打开
[管理菜单](/docs/guide/update-uninstall#the-radpanel-management-menu)，然后
...
`db/` 卷保存 SQLite 数据库（`/etc/radpanel/radpanel.db`），`cert/`
...
高级用户可从[发布页面](https://github.com/radinmovafaghh-coder/radpanel/releases)
下载适用于你架构的发布归档包，解压后
...
- **SQLite**（默认）——位于 `/etc/radpanel/radpanel.db` 的单个文件。无需任何配置。
...
仓库的 [`deploy/`](https://github.com/radinmovafaghh-coder/radpanel/tree/main/deploy)
