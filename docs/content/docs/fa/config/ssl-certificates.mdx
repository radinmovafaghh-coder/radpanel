---
title: گواهی‌های SSL
description: دریافت و تمدید گواهی‌های TLS برای پنل و ورودی‌های radpanel — با منوی ACME در radpanel (دامنه یا IP خام)، گواهی wildcard از طریق Cloudflare DNS-01، یا Certbot دستی.
icon: ShieldCheck
---
...
- **منوی `radpanel`** — کلاینت داخلی [ACME](https://en.wikipedia.org/wiki/Automatic_Certificate_Management_Environment).
...
## منوی SSL در `radpanel` (Let's Encrypt)

`radpanel` را اجرا کنید و **`20` — SSL Certificate Management** را انتخاب کنید. این گزینه
...
`radpanel` ← `20` ← **Get SSL (Domain)**، سپس دامنه را وارد کنید. acme.sh گواهی را
...
`radpanel` ← **`21` — Cloudflare SSL Certificate**. هنگام پرسش، **`t`** را برای یک
