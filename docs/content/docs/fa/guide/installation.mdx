---
title: نصب
description: نصب radpanel با اسکریپت رسمی (نسخهٔ پایدار، نسخهٔ مشخص یا dev-latest)، نصب خودکار/cloud-init یا Docker — و انتخاب میان SQLite و PostgreSQL.
icon: Download
---

‏radpanel روی طیف گسترده‌ای از توزیع‌های Linux اجرا می‌شود — Ubuntu، Debian، Armbian،
...
<Callout type="warn">
  ...
  سرویس نصب می‌کند، دستور مدیریتی `radpanel` را راه‌اندازی می‌کند و پنل را در زمان
  بوت فعال می‌سازد.
</Callout>
...
دستور مدیریتی `radpanel` را نصب می‌کند.
...
پس از پایان نصب، مشخصات ورود چاپ‌شده را یادداشت کنید و دستور `radpanel` را اجرا کنید تا
[منوی مدیریت](/docs/guide/update-uninstall#the-radpanel-management-menu) باز شود، سپس
...
والیوم `db/` پایگاه‌دادهٔ SQLite (`/etc/radpanel/radpanel.db`) را نگه می‌دارد و `cert/`
...
[صفحهٔ releases](https://github.com/radinmovafaghh-coder/radpanel/releases) دانلود کنید، آن را
...
- **SQLite** (پیش‌فرض) — یک فایل واحد در مسیر `/etc/radpanel/radpanel.db`. بدون نیاز به هیچ تنظیماتی.
...
`/etc/radpanel/install-result.env` بنویسد:

```bash
XUI_NONINTERACTIVE=1 bash <(curl -Ls https://raw.githubusercontent.com/radinmovafaghh-coder/radpanel/master/install.sh)
```

دایرکتوری [`deploy/`](https://github.com/radinmovafaghh-coder/radpanel/tree/main/deploy) در مخزن
...
<Cards>
  ...
  <Card
    ...
    description="منوی radpanel، به‌روزرسانی‌ها و حذف."
  />
  <Card
    title="REALITY"
