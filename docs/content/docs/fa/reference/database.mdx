---
title: پایگاه داده
description: بک‌اندهای ذخیره‌سازی radpanel — SQLite (پیش‌فرض) و PostgreSQL — مسیر پایگاه داده، استخر اتصال و مهاجرت از SQLite به PostgreSQL.
icon: Database
---

radpanel همه چیز را — inboundها، کلاینت‌ها و تنظیمات — در یک پایگاه داده ذخیره می‌کند.
بک‌اند را هنگام نصب انتخاب می‌کنید؛ هر دو کاملاً پشتیبانی می‌شوند.

## SQLite (پیش‌فرض)

یک فایل واحد در مسیر `/etc/radpanel/radpanel.db`. بدون نیاز به هیچ راه‌اندازی، ایده‌آل برای
استقرارهای کوچک و متوسط. این پوشه با
[`RADPANEL_DB_FOLDER`](/docs/reference/env-vars#database) قابل تنظیم است (در ویندوز به‌طور
...
سپس `XUI_DB_TYPE` و `XUI_DB_DSN` را در `/etc/default/radpanel` تنظیم کرده و سرویس را مجدداً
راه‌اندازی کنید:
