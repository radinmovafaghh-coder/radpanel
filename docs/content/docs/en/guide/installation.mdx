---
title: Installation
description: Install radpanel via the official script (stable, pinned, or dev-latest), unattended/cloud-init, or Docker — and choose SQLite or PostgreSQL.
icon: Download
---

radpanel runs on a wide range of Linux distributions — Ubuntu, Debian, Armbian,
...
<Callout type="warn">
  ...
  sets up the `radpanel` management command, and enables the panel on boot.
</Callout>
...
and installs the `radpanel` management command.
...
When it finishes, note the printed login details and run `radpanel` to open the
[management menu](/docs/guide/update-uninstall#the-radpanel-management-menu), then
...
The `db/` volume holds the SQLite database (`/etc/radpanel/radpanel.db`) and `cert/`
...
[releases page](https://github.com/radinmovafaghh-coder/radpanel/releases), extract it, and run
...
- **SQLite** (default) — a single file at `/etc/radpanel/radpanel.db`. Zero setup.
...
<Cards>
  ...
  <Card
    ...
    description="The radpanel menu, updates, and removal."
  />
  <Card
    title="REALITY"
