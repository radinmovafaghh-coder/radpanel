---
title: First Login
description: Find your generated radpanel credentials, reach the panel, enable two-factor auth, and harden it before exposing anything.
icon: KeyRound
---
...
`docker exec -it <container> radpanel setting -show`.
...
radpanel supports TOTP two-factor auth (compatible with Google Authenticator, Aegis,
...
step or with `radpanel setting -resetTwoFactor`.
...
Use a valid certificate (via the `radpanel` menu's SSL management, or a reverse
