...
description: Get and renew TLS certificates for the radpanel panel and inbounds — with the radpanel ACME menu (domain or bare IP), a Cloudflare DNS-01 wildcard, or manual Certbot.
icon: ShieldCheck
---
...
- **The `radpanel` menu** — built-in [ACME](https://en.wikipedia.org/wiki/Automatic_Certificate_Management_Environment)
...
## The `radpanel` SSL menu (Let's Encrypt)

Run `radpanel` and choose **`20` — SSL Certificate Management**. It drives
...
`radpanel` → `20` → **Get SSL (Domain)**, then enter the domain. acme.sh requests
...
`radpanel` → **`21` — Cloudflare SSL Certificate**. When asked, choose **`t`** for an
