...
description: Shape egress in radpanel — WARP, NordVPN, PIA WireGuard, outbound subscriptions, routing rules, and load balancers.
icon: Route
---

Inbounds accept clients; **outbounds** decide where their traffic goes next.
radpanel can route traffic through Cloudflare WARP, NordVPN, Private Internet Access
...
shapes radpanel uses: **VLESS is the flat form** (`address`/`port`/`id`/`flow`/
...
WARP lets your server egress through Cloudflare's network. radpanel can register a
...
From the panel's WARP controls, register an account. radpanel fills the outbound's
...
radpanel can fetch NordVPN (NordLynx/WireGuard) credentials from an access token (or
...
radpanel can sign in with a PIA username and password, list countries/regions/servers
