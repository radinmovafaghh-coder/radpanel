---
title: Discord Bot
description: Connect a Discord bot to radpanel to receive real-time Embed notifications in a channel for panel events (service crashes, node status, CPU/RAM load, and login attempts).
icon: Bot
---

radpanel provides comprehensive Discord integration: real-time event notifications via the event bus (`EventBus`), periodic scheduled health reports with database backups, and interactive commands via the Discord Gateway.
...
2. Click **New Application** at the top right, enter a name (e.g., `radpanel Notifier`), and confirm.
...
1. In the radpanel panel, open **Panel Settings** $\rightarrow$ **Discord Bot** (or navigate to `/settings#discord`).
2. Under **General**:
...
3. Under **Notifications**:
   ...
   - Optionally toggle **Database Backups** to automatically attach `radpanel.db` with periodic reports.
...
| `!backup` | Download current database backup file (`radpanel.db`) and `config.json`. |
...
| `discordBotBackup` | `false` | Whether to attach database backup (`radpanel.db`) to reports. |
