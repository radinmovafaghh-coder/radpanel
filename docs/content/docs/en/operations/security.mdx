---
title: Security
description: Harden radpanel — panel auth and 2FA, Fail2ban IP limits, firewall rules, BBR, and staying current.
icon: ShieldCheck
---
...
radpanel installs and configures for you (enabled by default on script installs, and
on Docker via `XUI_ENABLE_FAIL2BAN=true`).

Manage it from the `radpanel` menu (**22 — IP Limit Management**): install/configure,
change the ban duration (default **30 minutes**), ban/unban an IP, view ban logs,
and check status. Under the hood:

- The jail is named **`3x-ipl`**; ban logs live at `/var/log/radpanel/3xipl.log` and
  `/var/log/radpanel/3xipl-banned.log` (also via `radpanel banlog`).
...
port, and your inbound ports. The `radpanel` menu (**23 — Firewall Management**) wraps
...
The `radpanel` menu (**26 — Enable BBR**) toggles Google's BBR congestion control
(`net.ipv4.tcp_congestion_control = bbr`, `net.core.default_qdisc = fq`), which
often improves throughput on congested links.

## Keep current

Update radpanel and Xray-core regularly — security fixes land in new releases. Watch
the [releases page](https://github.com/radinmovafaghh-coder/radpanel/releases) and see
