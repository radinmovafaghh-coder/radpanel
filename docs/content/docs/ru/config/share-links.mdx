...
description: Форматы ссылок для обмена в radpanel (vless, vmess, trojan, ss, hysteria2, mtproto), переменные шаблона примечания и инспектор ссылок в браузере.
icon: Link
---

radpanel генерирует **ссылку для обмена** (и QR-код) для каждого клиента. Клиентские приложения,
