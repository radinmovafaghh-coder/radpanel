---
title: База данных
description: Хранилища данных radpanel — SQLite (по умолчанию) и PostgreSQL — путь к базе данных, пул соединений и миграция с SQLite на PostgreSQL.
icon: Database
---

radpanel хранит всё — входящие подключения, клиентов, настройки — в базе данных.
Бэкенд выбирается во время установки; оба являются полноценными.

## SQLite (по умолчанию)

Один файл по пути `/etc/radpanel/radpanel.db`. Никакой настройки не требуется, идеально
подходит для небольших и средних развёртываний. Папку можно изменить с помощью
[`RADPANEL_DB_FOLDER`](/docs/reference/env-vars#database) (в Windows по умолчанию она
...
переменные окружения, которые инсталлятор записывает в `/etc/default/radpanel`:
