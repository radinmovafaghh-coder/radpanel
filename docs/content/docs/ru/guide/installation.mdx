---
title: Установка
description: Установите radpanel с помощью официального скрипта (стабильная, фиксированная или dev-сборка), в автоматическом режиме / через cloud-init либо в Docker — и выберите SQLite или PostgreSQL.
icon: Download
---

radpanel работает на широком спектре дистрибутивов Linux — Ubuntu, Debian, Armbian,
...
<Callout type="warn">
  ...
  устанавливает службу, настраивает команду управления `radpanel` и включает
...
настраивает службу и устанавливает команду управления `radpanel`.
...
bash <(curl -Ls https://raw.githubusercontent.com/radinmovafaghh-coder/radpanel/master/install.sh) v3.4.0
```
...
По завершении запишите выведенные данные для входа и выполните `radpanel`, чтобы
открыть [меню управления](/docs/guide/update-uninstall#the-radpanel-management-menu),
...
docker run -d \
  ...
  -v $PWD/db/:/etc/radpanel/ \
  -v $PWD/cert/:/root/cert/ \
  --network=host \
  --restart=unless-stopped \
  --name radpanel \
  ghcr.io/radinmovafaghh-coder/radpanel:latest
```

Том `db/` хранит базу данных SQLite (`/etc/radpanel/radpanel.db`), а `cert/` — TLS-сертификаты,
...
[страницы релизов](https://github.com/radinmovafaghh-coder/radpanel/releases), распаковать его и
...
- **SQLite** (по умолчанию) — единственный файл по пути `/etc/radpanel/radpanel.db`. Без
...
записав их в `/etc/radpanel/install-result.env`:

```bash
XUI_NONINTERACTIVE=1 bash <(curl -Ls https://raw.githubusercontent.com/radinmovafaghh-coder/radpanel/master/install.sh)
```

В каталоге [`deploy/`](https://github.com/radinmovafaghh-coder/radpanel/tree/main/deploy)
...
<Cards>
  ...
  <Card
    ...
    description="Меню radpanel, обновления и удаление."
  />
  <Card
    title="REALITY"
