// Package logger provides logging functionality for the radpanel panel with
...
const (
	...
	logFileName      = "radpanel.log"            // Log file name
...
func init() {
	logger.Store(logging.MustGetLogger("radpanel"))
}
...
func InitLogger(level logging.Level) {
	newLogger := logging.MustGetLogger("radpanel")
	...
	leveledBackend.SetLevel(level, "radpanel")
	...
	if fileBackend := initFileBackend(); fileBackend != nil {
		...
		leveledBackend.SetLevel(logging.DEBUG, "radpanel")
		backends = append(backends, leveledBackend)
	}
