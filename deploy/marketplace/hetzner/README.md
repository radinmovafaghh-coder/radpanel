# radpanel on Hetzner Cloud

Hetzner Cloud does **not** have a third-party image marketplace the way AWS does.
Ship radpanel via **cloud-init**: each instance installs non-interactively and
...
radpanel non-interactively and generates unique per-instance credentials.
...
```bash
ssh root@<server-ip> 'cat /etc/radpanel/install-result.env'
```
...
is essentially a documented cloud-init config plus metadata. To propose radpanel as
